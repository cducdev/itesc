...
export function ReportActions({
	...
	const handleSaveToKnowledgeBase = () => {
		...
		if (success) {
			toast({
				title: "Đã lưu vào kho kiến thức",
				description: "Báo cáo đã được lưu để tham khảo sau",
			});
		}
	};
	...
	const handleCopy = async () => {
		try {
			...
			const filteredSources =
				report.usedSources &&
				report.usedSources.length > 0 &&
				report.sources
					? report.sources.filter((_, index) =>
							report
								.usedSources!.map((num) => num - 1)
								.includes(index)
					  )
					: report.sources;
		...
		<div className={`flex gap-2 ${className}`}>
			...
			{!hideKnowledgeBase && (
				<Button
					...
					title={isReportSaved ? "Đã lưu" : "Lưu vào kho kiến thức"}
				>
					<Brain className="h-4 w-4" />
					<span className="hidden sm:inline">
						{isReportSaved ? "Đã lưu" : "Lưu vào kho kiến thức"}
					</span>
				</Button>
			)}
			<DropdownMenu>
