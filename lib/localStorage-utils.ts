...
export function clearAllFlowProjects(): void {
	localStorage.removeItem("IT-ESC-flow-projects");
	localStorage.removeItem("IT-ESC-current-project");
}
...
export function exportFlowProjects(): string {
	const projects = localStorage.getItem("IT-ESC-flow-projects") || "[]";
	return projects;
}
...
export function importFlowProjects(jsonProjects: string): boolean {
	try {
		...
		localStorage.setItem("IT-ESC-flow-projects", jsonProjects);

		// If there's a current project ID, verify it still exists in the imported data
		const currentProjectId = localStorage.getItem("IT-ESC-current-project");
		if (currentProjectId) {
