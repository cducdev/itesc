	...
	const handleGenerateReport = useCallback(
		async (
			...
			try {
				...
				const validResults = contentResults.filter((r) =>
					r.content?.trim()
				);
				if (validResults.length === 0) {
					throw new Error(
						"No valid content found in selected results"
					);
				}
				...
				setTimeout(() => {
					if (simpleSave) {
						...
						console.log(
							"🔥 FORCE SAVED REPORT DATA TO LOCALSTORAGE!"
						);
					} else {
	...
	const startResearch = useCallback(
		async (
			...
			try {
				...
				const searchResults = data.webPages.value.map(
					(result: any) => ({
						id:
							result.id ||
		...
		async (selectedReports: string[]) => {
			...
			try {
				...
				if (reportsToConsolidate.length < 2) {
					throw new Error(
						"Need at least 2 valid reports to consolidate"
					);
				}
...
function Flow() {
		...
		(reportId: string) => {
			setSelectedReports((currentSelectedReports) => {
				const isCurrentlySelected =
					currentSelectedReports.includes(reportId);
	...
	const renderNavigation = () => (
		<nav className="border-b border-gray-800 bg-black/80 shadow-sm backdrop-blur-sm">
	...
	const renderControls = () => (
		<div className="p-4 bg-black/80 border-b border-gray-800 backdrop-blur-sm">
				...
				<div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
					<div className="flex flex-col sm:flex-row items-start sm:items-center gap-2 w-full sm:w-auto">
						...
						<ModelSelect
							...
							triggerClassName="w-full sm:w-[200px] bg-black/50 text-white border-gray-700"
						/>
					</div>
					<Button
						...
						variant="outline"
						className="gap-2 w-full sm:w-auto sm:ml-auto text-white border-gray-700 bg-black/50 hover:bg-black/70"
					>
						{isConsolidating ? (
							<>
	...
	const renderErrorNotification = () => {
		...
		return (
			<div className="absolute top-0 left-0 right-0 z-10 p-3 bg-red-900/50 border-b border-red-800">
	...
	return (
		<div className="h-screen flex flex-col bg-[#c78b8b]">
			{renderNavigation()}
			{renderControls()}
			<div className="flex-1 w-full h-0 relative overflow-hidden">
				...
				<ReactFlow
					...
					edgeTypes={edgeTypes}
					fitView={
						nodes.length > 0 && !currentProject?.nodes?.length
							? true
							: false
					}
					minZoom={0.1}
				...
				>
					<MiniMap
						nodeStrokeWidth={3}
						className="!bottom-4 !right-4 !left-auto bg-black/50 border border-gray-800"
						pannable
						zoomable
					/>
					<Background color="#374151" />
					<Controls
						className="!top-4 !right-4 !left-auto !bottom-auto bg-black/50 border border-gray-800"
