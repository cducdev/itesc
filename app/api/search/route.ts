	...
	try {
		...
		if (provider === "exa") {
			...
			if (!exaApiKey) {
				return NextResponse.json(
					{
						error: "Exa search API is not properly configured. Please check your environment variables.",
					},
					{ status: 500 }
				);
			}
			...
			if (!googleApiKey || !googleCx) {
				return NextResponse.json(
					{
						error: "Google search API is not properly configured. Please check your environment variables.",
					},
					{ status: 500 }
				);
			}
			...
			const dateRestrict = getGoogleDateRestrict(
			...
			if (!googleResponse.ok) {
				...
				if (errorData?.error?.message?.includes("Quota exceeded")) {
					return NextResponse.json(
						{
							error: "Daily search limit reached. Please try again tomorrow or contact support for increased limits.",
						},
						{ status: 429 }
					);
				}
				...
				return NextResponse.json(
					{
						error: "An error occurred while fetching search results. Please try again later.",
					},
